...
poisson_glm_workflow <-
    workflow() %>%
    add_recipe(base_recipe) %>%
    add_model(poisson_glm_model)

poisson_glm_fit <-
    fit(poisson_glm_workflow, data = training)

poisson_glm_fit

poisson_w_pop <- glm(Count_Asphyxia ~ Count_OPIOID_Death + publicinsurance + population,
        data = training)

summary(poisson_w_pop)
```
...
The constant term is the prediction if `Count_OPIOID_Death` = 0 and `centered_public_insurance` = 0. In this case, because we’ve centered `public_insurance`, it implies that $e^{-1.816} = 0.163$, is the predicted number of asphyxiation deaths in a census tract with no opioid-related deaths and with the mean percentage of residents holding public insurance.
