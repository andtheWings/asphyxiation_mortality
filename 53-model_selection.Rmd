...
```{r}
glm(
    formula = suid_count ~ pop_under_five, 
...
```{r}
MASS::glm.nb(
    formula = suid_count ~ (pop_under_five + public_insurance + count_opioid_death + white)^2,
    data = tar_read(suid)
) |> 
...
plot_rootogram(nb_model_suid_count_per_tract)
```
